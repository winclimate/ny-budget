...
Let $q_{ij}(p)$ be the quantity of product $i$ which is purchased in market $j$ at price $p$. $q_{ij}$ is a function, whose parameters we need to estimate from some combination of data and heuristics. Keeping with a standard log-log form of the demand curve with constant price elasticity of demand, we assume that $q_{ij}(p) = X_{ij} p^{-\eta_{ij}}$.$\eta_{ij}$ is the price elasticity of demand for product $i$ in market $j$. 

$X_{ij}$ is a scaling factor; we can calculate it with some algebra by knowing the the quantity of people who would purchase the product if the price was de minimis, say at \$100 (that is, the total addressable market). If $M$ is the total adressable market, then

$$
\begin{align*}
M_{ij}&=X_{ij} 100^{-\eta_{ij}}\\
\frac{M_{ij}}{100^{-\eta_{ij}}}&=X_{ij}\\
100^{\eta_{ij}}M_{ij}&=X_{ij}
\end{align*}
$$

(Note that usually these things are parameterized with more $\log$s thrown in, so that a price elasticity of demand of .001 by the above formulation might be listed as 6.9 in a table or coming out of a regression.)

For example, if $X_{11} = 1,000,000$ middle-income households could buy a heat pump next year, and $\eta_{11} = .001$ is the price elasticity of demand for that market, then the number of heat pumps purchased by middle-income households as a function of price would be described as follows (note that we're putting price on the x axis and quantity on the y axis, which is not normal in economics but more sensible to me and probably to other outsiders):

```{r}
# Define our quantity as a function of price, M and eta
q <- function(M, eta, p) {(100^eta*M) * p^(-eta)}
```

To visualize:
```{r}
ggplot() +
  geom_function(fun = ~q(1000000, 1, .x), xlim = c(100,10000)) +
  xlab("Price") +
  ylab("Qty")
```

That is, at a price of $5000 each, we expect `r q(1000000, .001, 5000)` households to buy heat pumps next year.
...
So at a price of $5000 per unit, we should expect `r g_1 * q(1000000, .001, 5000)` kg of CO2 to be saved.
...
To do this, we pick a base price $p_i$ to work off of. To continue this example of heat pumps for middle income households, let's say that our base price will be \$10,000, and our possible subsidy levels will run from 0 up to \$10,000. How much CO2 reduction can we expect at each subsidy level?
...
As we get close to complete subsidy, we save a ton of CO2, but it's also going to be extremely expensive to subsidize everyone's heat pump. What's the best use of our budget?

### Optimization

Let's say our total budget $B$ is \$10,000,000. How should we select all of the incentive levels so that we get the biggest GHG reduction for our given budget?

We start by expanding out the possible program subsidy levels and their resulting cost and GHG savings. Note again that all of these adressable market numbers and elasticities are made up, but we could estimate them from real data.
...
```{r}
f.dir.2 <- c("<=", replicate(num_programs, "<="))

optim.2 <- lp(direction = "max",
              f.obj,
              f.con,
              f.dir.2,
              f.rhs,
...
Now how close did we get to the budget, given these program options?

```{r}
sum(s$s_total[optim.2$solution == 1])/B
```



And which programs to fund?

```{r}
s$name[optim.2$solution == 1]
```

Now how much millions of kg of GHG will we save per year?

```{r}
sum(s$g_saved[optim.2$solution == 1]) / 1e6
```
